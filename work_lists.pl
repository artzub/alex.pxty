%%% ќЅџ„Ќџ≈ —ѕ»— » Ч обычно пердставление %%%
% длина списка
llen([], 0). %условие выхода из рекурсии. факт что пустой список длина равна 0.
%тоже самое
% llen([], 0) :- true.

% отдел€ем голову в аннонимную переменую и запускаем рекурсивно
% разделение хвоста пока не получим факт что пустой список равен 0 на
% каждом шаге добавл€ем одик в длине
llen([_|T], L) :-
	llen(T, L1),
	L is L1 + 1.
...
% отдел€ем хвотс и провер€ем истиность выше указаного факта
lmem(I, [_|T]) :-
	lmem(I, T).

% сли€ние, деление и проверка эдентичности списков в общем предикарт
% append
...
% только правда Res формируетс€ в обратном пор€дке
...
%%% ќЅџ„Ќџ≈ —ѕ»— » Ч пор€дковое представление %%%
% элементами списка €вл€ютс€ функторы
% e(i, val)
% e(i, j, val)
% по сутит это теже матрици что в с++
% [1, 4, 7, 10] = [e(1, 1), e(2, 4), e(3, 7), e(4, 10)]
% [[1, 2], [3, 4]] = [e(1, 1, 1), e(1, 2, 2), e(2, 1, 3), e(2, 2, 4)]
% дл€ них подход€т описанные выше придекарты.
ldel_n_ord([], _, []).
